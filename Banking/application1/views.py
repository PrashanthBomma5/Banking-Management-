...
def resetpassword(request):
    if request.method == 'POST':
        ...
        return redirect('login')  # Redirect to a success page or the reset password page

    return render(request, 'settings.html')  # Assuming 'reset.html' is the template for resetting the password
# def reset(req):
...
def userTransactions(request, id):
    userTransactions = Account.objects.get(id=id)
    usertrans = Transactions.objects.filter(account_id=userTransactions.accountNumber)
    received_transactions = Transactions.objects.filter(account_to=userTransactions.accountNumber, Transaction_type="Received")
    context = {
        'userTransactions': userTransactions,
        'id': id,
        'usertrans': usertrans,
